...
export function CreateTaskDialog({ open, config, onOpenChange, onCreate }: { open: boolean; config: StreamConfig; onOpenChange(open: boolean): void; onCreate(input: CreateTaskInput): Promise<void> }) {
  ...
  useEffect(() => {
    if (open && !wasOpen.current) { setTitle(''); setDescription(''); setStatus(config.statuses[0] || ''); setAssignee(''); setLabels(''); setError(''); }
    wasOpen.current = open;
  }, [open, config.statuses]);
  useEffect(() => { if (open && status && !config.statuses.includes(status)) setStatus(config.statuses[0] || ''); }, [open, status, config.statuses]);
